...
s.every '1h' do
  ...
  @patients.each do |patient|
    ...
    if p.nil?
      ...
      User.create!(password: "n3wp4t13n1", password_confirmation: "n3wp4t13n1", elation_id: patient["id"], email:  patient["email"].blank? ? "example@email.com" : patient["email"], name:  patient["first_name"], elation_payload: patient.to_json)
    else
      p.elation_payload = patient.to_json
      p.save
    end
  end
end